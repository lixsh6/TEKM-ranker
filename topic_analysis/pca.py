...
def draw_topic_points(embeddings1,embeddings2):
    ...
    train_data = np.concatenate((embeddings1,embeddings2,topic_embeddings[:1]),axis=0)
    tsne_model.fit(train_data)

    tsne_X1 = tsne_model.transform(embeddings1)
    tsne_X2 = tsne_model.transform(embeddings2)
    topic_X = tsne_model.transform(topic_embeddings[:1])
    fig = plt.figure()
    ...
    plt.scatter(tsne_X2[:, 0], tsne_X2[:, 1], marker='o', s=5,c='b', cmap=plt.cm.Spectral)

    #x_range = 5
    #plt.xlim(-x_range, x_range)
    fig.savefig('comparison.png', dpi=fig.dpi)
